...
const actions = {
  ...
  logout(context) {
    ...
    resetRouter()
    // 设置权限模块下路由为初始状态
    // vuex 子模块调用子模块的 action 方法
    // 没有命名空间的所有的 mutations 和 action 都是挂在全局上的，我们可以直接调用
    // 有命名空间的子模块如何调用另一个有命名空间的子模块的 mutations
    // 有命名空间的 context 并不是全局的 context，我们可以 第三个参数给 {root: true}, 表示调用根级的mutations 或者 actions 。
